...
import { Building2, Save, Loader2, Image as ImageIcon, CheckCircle2, X, ChevronLeft, ShieldCheck, Award } from "lucide-react";
...
export default function CabinetProfilePage() {
  ...
  const handleCarimboChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0] && storage && cabinetId) {
      ...
      setSaving(true);
      const storageRef = ref(storage, `gabinetes/${cabinetId}/carimbo_oficial`);
    ...
    <div className="min-h-screen bg-background text-white">
      ...
      <main className="container mx-auto px-4 py-8 sm:py-12">
        <header className="mb-10">
          ...
          <p className="text-muted-foreground text-[10px] font-black uppercase tracking-widest mt-1">Configure o seu carimbo de assinatura oficial.</p>
        </header>
            ...
            <Card className="bg-white/5 border-white/5 shadow-2xl relative overflow-hidden">
              ...
              <CardHeader>
                <CardTitle className="text-[10px] font-black uppercase tracking-widest text-primary flex items-center gap-2">
                  <ShieldCheck size={16} /> Carimbo Oficial (Selo de Assinatura)
                </CardTitle>
              ...
              <CardContent className="space-y-8 p-10">
                <div className="flex flex-col items-center justify-center border-2 border-dashed border-white/10 rounded-full w-80 h-80 mx-auto bg-black/40 relative group hover:border-primary/40 transition-all overflow-hidden">
                  {carimboPreview ? (
                ...
                <div className="p-6 bg-primary/10 rounded-2xl border border-primary/20 flex gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] text-white/60 uppercase font-bold mt-1">Para o melhor resultado, envie uma imagem com fundo transparente (PNG). O sistema irá posicionar o carimbo automaticamente no final dos seus ofícios.</p>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <aside className="space-y-6">
            <Card className="bg-white/5 border-white/5 shadow-2xl overflow-hidden">
              <CardHeader className="bg-white/5 border-b border-white/5">
                <CardTitle className="text-[10px] font-black uppercase tracking-widest text-primary">Status do Vereador</CardTitle>
              </CardHeader>
              <CardContent className="pt-6 space-y-4">
                ...
                <div className="space-y-1">
                  <p className="text-[8px] font-black uppercase text-muted-foreground tracking-widest">Identificador</p>
                  <p className="text-[10px] font-mono text-white/40">{cabinetId}</p>
                </div>
              </CardContent>
            </Card>
          </aside>
